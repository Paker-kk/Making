...
const ToolButton: React.FC<{
    ...
    <button
        ...
        className={`p-2 rounded-md transition-colors duration-200 text-neutral-500 ${
            isActive ? 'bg-neutral-100 text-neutral-600' : 'hover:bg-neutral-50'
...
export const Toolbar: React.FC<ToolbarProps> = ({
    ...
    // 计算 left 位置：图层展开时在图层面板右边，收起时紧贴左边
    const leftPosition = isLayerPanelExpanded ? 288 : 16; // 16px (left) + 256px (panel) + 16px (gap) or 16px (left-4)
        ...
        return (
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={onCancelCrop} className="px-3 py-1.5 text-sm rounded-md bg-neutral-50 hover:bg-white border border-neutral-200 text-neutral-700">{t('toolbar.crop.cancel')}</button>
                    <button onClick={onConfirmCrop} className="px-3 py-1.5 text-sm rounded-md bg-neutral-900 text-white hover:brightness-110">{t('toolbar.crop.confirm')}</button>
                </div>
            </div>
        )
    }
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col items-center gap-2 flex-grow">
                ...
                <div className="w-10 h-px bg-neutral-200"></div>
                <input type="color" aria-label={t('toolbar.strokeColor')} title={t('toolbar.strokeColor')} value={drawingOptions.strokeColor} onChange={(e) => setDrawingOptions({ ...drawingOptions, strokeColor: e.target.value })} className="w-8 h-8 p-0 border border-neutral-300 rounded-md cursor-pointer bg-transparent" />
                ...
                <div className="w-10 h-px bg-neutral-200"></div>
                <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*" className="hidden" aria-label={t('toolbar.upload')} title={t('toolbar.upload')} />
